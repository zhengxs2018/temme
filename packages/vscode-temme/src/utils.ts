...
export async function placeOutputDocInAnotherViewColumnIfNotVisible(temmeDoc: TextDocument, outputDoc: TextDocument) {
  const visibleDocs = new Set(window.visibleTextEditors.map(editor => editor.document));
  if (!visibleDocs.has(outputDoc)) {
    const activeViewColumn = window.activeTextEditor ? window.activeTextEditor.viewColumn : ViewColumn.One;
    await window.showTextDocument(outputDoc, activeViewColumn === ViewColumn.Two ? ViewColumn.One : ViewColumn.Two);
